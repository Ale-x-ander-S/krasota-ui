...
export class AuthService {
  ...
  getProfile(): Observable<User> {
    ...
    return this.http.get<User>('/api/v1/users/me', { headers });
  }
  ...
  updateProfile(profileData: Partial<User>): Observable<User> {
    const token = this.getToken();
    const headers = { 'Authorization': `Bearer ${token}` };
    return this.http.put<User>('/api/v1/users/me', profileData, { headers }).pipe(
      tap(updatedUser => {
  ...
  updateEmailStatus(isEmailActive: boolean): Observable<any> {
    ...
    return this.http.put<any>('/api/v1/users/me/email-status', body, { headers }).pipe(
      tap(() => {
        // Обновляем локальные данные пользователя
